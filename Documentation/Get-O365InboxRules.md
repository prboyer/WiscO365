...
### -msoluid
Unique ID of the account for which to get inbox rules (e.g.
foo_bar)
...
### failure: null
### success: An array of Inbox rules with the following key/values: name (string), priority (integer), conditions (array, list of conditions that triggers the rule), actions (array, actions to take on message); returns empty array if none
## NOTES
